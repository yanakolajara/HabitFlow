...
async function addHabitToUser(userId, habitId) {
  try {
    const habitToUser = await Axios.post(
      `/users/${userId}/habits/user-habits/${habitId}`
    );
    return habitToUser;
  } catch (error) {
    return error;
...
async function getHabitStats(userId, habitId) {
  try {
    const habitStats = await Axios.get(
      `/users/${userId}/habits/stats/${habitId}`
    );
    return habitStats;
  } catch (error) {
    return error;
...
async function createHabitStats(
  user_id,
  habit_id,
  day,
  month,
  ...
  try {
    const newStat = await Axios.post(
      `/users/${user_id}/habits/stats/${habit_id}`,
      {
        day: day,
        month: month,
        year: year,
